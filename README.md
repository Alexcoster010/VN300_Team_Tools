...
The current desktop release is `v0.15.3`. Its GitHub release assets are Windows installers and checksums. The Raspberry Pi Trixie offline bundle is built locally from a committed checkout; it is not published as a GitHub release asset. Once installed, the desktop app checks GitHub for future desktop releases and can install them from inside the app.
