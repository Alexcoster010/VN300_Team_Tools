...
**Raspberry Pi OS Bookworm 64-bit (arm64), system Python 3.11**, Desktop or Lite,
on Raspberry Pi hardware. The installer explicitly rejects armhf/armv7, Trixie,
other Python ABIs, and non-Pi systems before installing any packages. This is a
packaged compatibility target, not a claim of physical Pi validation.

Flash a stock Bookworm 64-bit image; in Raspberry Pi Imager configure a login,
...
`/home/vectornav`. Core OS facilities must already exist: Python 3.11 (including
stdlib venv), libc6 >=2.34, libudev1 >=183, systemd, sudo, iproute2, apt/dpkg,
...
The ARM64 packages are can-utils 2020.11.0-1, libusb-1.0-0 1.0.26-1,
liblgpio1 and python3-lgpio 0.2.2-1~rpt1. Their complete non-base dependency
...
ARM64/CPython 3.11 wheel; the other 15 wheels are architecture-independent.
...
$Zip = 'VN300_Offline_Bookworm_arm64.zip'
...
```sh
cd ~
sha256sum -c VN300_Offline_Bookworm_arm64.zip.sha256
python3 -m zipfile -e VN300_Offline_Bookworm_arm64.zip .
cd ~/VN300_Offline
...
hotplug on Bookworm's NetworkManager, create a dedicated systemd/udev pair:
...
- Unsupported OS/architecture: use Bookworm **64-bit**; do not force-install
  ARM64 packages on 32-bit or CPython 3.11 extensions into Trixie Python 3.13.
...
```sh
python3 packaging/build_pi_offline.py
# A later rebuild using only the previously downloaded, hashed cache:
python3 packaging/build_pi_offline.py --no-download
python3 -m zipfile -t dist/VN300_Offline_Bookworm_arm64.zip
```
