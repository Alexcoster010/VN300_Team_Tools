...
sudo usermod -aG dialout,gpio,netdev vectornav 2>/dev/null || true

if ! python3 -c "import serial, gpiozero, can" 2>/dev/null; then
  python3 -m pip install --user -r requirements-pi.txt || {
    echo "Could not install Python packages with pip."
    echo "Try: sudo apt install -y python3-serial python3-gpiozero python3-can"
    exit 1
  }
fi
