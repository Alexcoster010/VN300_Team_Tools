...
PAYLOAD = ['PI_LOGGER_VERSION', 'pi/vn300_button_logger.py',
           ...
           'pi/can_profile.example.json', 'pi/CAN_SETUP.md', 'OFFLINE_INSTALL_TRIXIE.md']
...
def main():
    ...
    ap.add_argument('--output', type=Path, default=ROOT / 'dist/VN300_Offline_Trixie_arm64_py313.zip')
    ap.add_argument('--cache', type=Path, default=ROOT / 'build/offline-cache-trixie')
    ...
    files.update({p: committed('packaging/offline-trixie/' + p) for p in SUPPORT})
