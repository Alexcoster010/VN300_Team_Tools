...
command -v python3 >/dev/null || fail 'System python3 is absent; reimage with Raspberry Pi OS Bookworm 64-bit.'
python3 -B "$ROOT/verify_manifest.py" "$ROOT"
. /etc/os-release
ARCH=$(dpkg --print-architecture)
PY=$(python3 -c 'import sys; print("%d.%d" % sys.version_info[:2])')
[ "${VERSION_CODENAME:-}" = bookworm ] && [ "$ARCH" = arm64 ] && [ "$PY" = 3.11 ] || fail "Unsupported OS/ABI: ${VERSION_CODENAME:-unknown}/$ARCH/Python $PY. This bundle requires Raspberry Pi OS Bookworm 64-bit (arm64), Python 3.11; armhf/armv7, Trixie and other ABIs need a separately built bundle. No packages installed."
...
for cmd in apt-get dpkg systemctl ip runuser visudo useradd usermod getent; do
    command -v "$cmd" >/dev/null || fail "Base OS command absent: $cmd. Use a stock Raspberry Pi OS Bookworm image."
done
...
apt-get --simulate --no-download --no-install-recommends --no-remove install "$ROOT"/debs/*.deb || fail 'Offline dependency preflight failed. Required baseline: Bookworm python3 3.11, libc6 >=2.34, libudev1 >=183. No network fallback is permitted.'
...
printf 'Reboot: sudo reboot\nCAN is opt-in; follow OFFLINE_INSTALL.md to configure can0 at 1000000 bit/s.\n'
